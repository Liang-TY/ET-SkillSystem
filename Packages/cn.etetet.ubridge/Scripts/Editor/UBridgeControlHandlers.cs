...
namespace ET
{
    ...
    /// <summary>
    /// YIUIAddControl: 通过克隆 YIUI/LoopScroll 模板 prefab 创建控件
    /// 先后查找两个模板目录：LoopScroll 包 > YIUI Framework 包
    /// </summary>
    public static class UBridgeYIUIAddControlHandler
    {
        [StaticField]
        private static readonly string[] s_TemplateDirs =
        {
        ...
        public static string Handle(string p)
        {
            var r = UBridgeJsonHelper.FromJson<YIUIAddControlRequest>(p);
            ...
            {
                resp.Error = 3; resp.Message = "Type required (e.g. LoopScrollVertical, UIBlockBG)";
                return UBridgeJsonHelper.ToJson(resp);
            }

            GameObject template = null;
            string foundPath = "";
            foreach (var dir in s_TemplateDirs)
            {
                foundPath = $"{dir}/{typeName}.prefab";
                template = AssetDatabase.LoadAssetAtPath<GameObject>(foundPath);
                if (template) break;
            }

            if (!template)
            {
                resp.Error = 3; resp.Message = $"Template not found: {typeName}. Searched in LoopScroll + YIUI template dirs.";
                return UBridgeJsonHelper.ToJson(resp);
            }

            var instance = UnityEngine.Object.Instantiate(template, parent.transform, false);
            instance.name = template.name.Replace("(Clone)", "");
